...
class ExperienceDto {
  ...
  @ApiProperty({ example: 'Developed scalable microservices...', required: false })
  @IsOptional()
  @IsString()
  @MaxLength(2000)
  description?: string;
...
class TemplateSelectionDto {
  @ApiProperty({ example: 'professional', enum: ['professional', 'creative', 'minimal', 'modern'] })
  @IsString()
  template: string;

  @ApiProperty({ example: 'ocean', enum: ['ocean', 'sunset', 'forest', 'lavender', 'rose', 'monochrome'] })
  @IsString()
  palette: string;
}
