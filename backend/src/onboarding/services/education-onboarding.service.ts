...
export class EducationOnboardingService {
  ...
  async saveEducation(resumeId: string, data: OnboardingData) {
    ...
    if (!educationStep || educationStep.noEducation || !educationStep.education?.length) {
      this.logger.log(educationStep?.noEducation ? 'User selected noEducation' : 'No education provided');
      return;
    }
