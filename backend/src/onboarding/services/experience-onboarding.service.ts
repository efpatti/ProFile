...
export class ExperienceOnboardingService {
  ...
  async saveExperiences(resumeId: string, data: OnboardingData) {
    ...
    if (!experiencesStep || experiencesStep.noExperience || !experiencesStep.experiences?.length) {
      this.logger.log(experiencesStep?.noExperience ? 'User selected noExperience' : 'No experiences provided');
      return;
    }
