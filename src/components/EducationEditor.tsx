...
export const EducationEditor: React.FC<EducationEditorProps> = ({ lang, initialItems, onSaved }) => {
  const { user } = useAuth();
  const { education, updateEducation, saveResume, isLoading } = useResumeStore();
  const items = initialItems || education;

  const handleSave = async (updatedItems: EducationItem[]) => {
    if (!user?.id) return;
