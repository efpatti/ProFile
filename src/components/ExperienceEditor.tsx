...
import { GenericEditor, AutoResizeTextarea } from "@/components/shared/GenericEditor";
...
export const ExperienceEditor: React.FC<ExperienceEditorProps> = ({ lang, initialItems, onSaved }) => {
  const { user } = useAuth();
  const { experiences, updateExperiences, saveResume, isLoading } = useResumeStore();
  const items = initialItems || experiences;

  const handleSave = async (updatedItems: Experience[]) => {
    if (!user?.id) return;
    updateExperiences(updatedItems);
        ...
        editing ? (
          <div className="space-y-2">
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => onChange({ ...exp, isCurrentJob: e.target.checked, endDate: e.target.checked ? null : exp.endDate })}
              />
            ...
            <input
              ...
              onChange={(e) => onChange({ ...exp, technologies: e.target.value.split(",").map(t => t.trim()) })}
              placeholder={lang === "pt-br" ? "Tecnologias (separadas por vírgula)" : "Technologies (comma-separated)"}
              className="w-full px-3 py-2 border rounded"
            />
            <AutoResizeTextarea
          ...
          <div>
            ...
            <div className="text-xs text-gray-500">
              {exp.startDate} - {exp.isCurrentJob ? (lang === "pt-br" ? "Presente" : "Present") : exp.endDate}
            </div>
            <div className="text-xs text-blue-600 mt-1">{exp.technologies.join(", ")}</div>
